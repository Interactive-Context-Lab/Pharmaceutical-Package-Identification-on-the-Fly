...
class pix2pix(object):
    ...
    def load(self, checkpoint_dir):
        ...
        # model_dir = "%s_%s_%s" % (self.dataset_name, self.batch_size, self.output_size)
        # model_dir = "%s_4_%s" % (self.dataset_name, self.output_size)
        # checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
    ...
    def demo_load(self, checkpoint_dir):
        print(" [*] Reading checkpoint...")
    ...
    def demo(self, args, img, img1, stat = True):
        """Test pix2pix"""
        if (stat):
            self.saver.restore(self.sess, "./checkpoint/BCN/pix2pix.model-64")
        else:
            # load testing input
            # print("Loading testing images ...")
            sample = [load_demo_data(img, is_test=True), load_demo_data(img1, is_test=True)]
            ...
            for i, sample_image in enumerate(sample_images):
                ...
                save_images(samples, [self.batch_size, 1],
                            './test/test_{:04d}.png'.format(idx))

    def test_IOU_load(self, checkpoint_dir, model_name):
        # print(" [*] Reading checkpoint...")
    ...
    def test_IOU(self, args, sample_images, model_name = None, stat=True):
        if (stat):
            init_op = tf.global_variables_initializer()
            self.sess.run(init_op)

            if self.test_IOU_load(self.checkpoint_dir, model_name):
                # print(" [*] Load SUCCESS")
                pass
            else:
                print(" [!] Load failed...")
        ...
        # sample_files = glob('./datasets/{}/{}/*.jpg'.format(self.dataset_name, test_dir))
        # for i in range(len(sample_files)):
        #     sample_files[i] = sample_files[i].replace("\\", "/")
        #
        # # # sort testing input
